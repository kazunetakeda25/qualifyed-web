      ...
      <div class="pageheader">
         ...
         <div class="page-bar">
            <ul class="page-breadcrumb">
               ...
               <li>
                  <a href="<?php echo site_url('manage_contacts_reason'); ?>">Manage Contacts Reason</a>
               </li>
            </ul>
         </div>
      </div>
         ...
         <div class="col-md-12">
            <section class="tile">
               ...
               <div class="tile-header dvd dvd-btm">
                  <h1 class="custom-font">Add Contacts Reason</h1>
                     <div class="comapny_size_add">
                  <form id="size_com" method="POST" action="<?php echo site_url('admin/contact_reason'); ?>">
                  <div class="row"> 
                  <div class="col-lg-6">            
                  <input type="text" id="myInput" class="form-control" name="name_re" placeholder=" Reason For Contact"></div>
                   <div class="col-lg-6">     
                  <input type="Submit" class="btn btn-primary" value="Submit" name="add_reason"></div>
                  </div>
                  </form>
                  </div>
                  <ul class="controls">
                        ...
                        <div class="row">
                           <div class="col-sm-12">
                              <table class="table table-custom dataTable no-footer" id="editable-usage" role="grid" aria-describedby="editable-usage_info">
                                 ...
                                 <tbody>
                                   <?php 
                                    if($master_contact_reason!="" && count($master_contact_reason) >0)
                                    {
                                   foreach ($master_contact_reason as $key => $value): ?>
                                    <tr class="gradeA odd" role="row">
                                       <td class="sorting_1"><?php echo $key+1  ; ?></td>
                                      <td class="edit_v"><?php echo $value['reason']; ?>
                                        <form  id="Edit_form_click_<?php echo $key+1 ?>" class="edit_form_oc" method="POST" >
                                            <div class="row"> 
                                            <div class="col-lg-6">            
                                            <input type="text" id="myInput"  class="form-control" value="<?php echo $value['reason']; ?>" name="name2" placeholder="Enter States Name "></div>
                                             <div class="col-lg-6">
  ...
  $('.edit_form_oc').submit(function(){
    ...
    if($.trim(misc) != '')
    {
      $.ajax({
        ...
        data : {table : 'master_contact_reason', col : 'reason',misc : misc,check_col_name : 'id', check_col_data : id  }
      }).then(function(res){
        ...
        swal("ERROR", "Please enter Contacts Reason", 'error');
    }
    return false;
  });
    $('.user_status').on('change', function(){
        ...
        $.ajax({
            method : "POST",
            url : "<?php echo site_url('admin/master_contact_reason_status'); ?>",
            data : {status : status , id : $(this).data('id')}
        }).then(function(){
            swal("Update Success", 'Enter Contacts Reason update successfully','success');
        })
    });
