...
<?php } ?>
<?php if(trim($this->session->flashdata('success'))!=''){?>
    <script type="text/javascript">
        swal("", "<?php echo $this->session->flashdata('success'); ?>", "success");
    </script>
   <?php } ?>
   <?php if(trim($this->session->flashdata('error'))!=''){?>
    <script type="text/javascript">
      ...
      <div class="pageheader">
         ...
         <div class="page-bar">
            <ul class="page-breadcrumb">
               ...
               <li>
                  <a href="<?php echo site_url('master_company_type'); ?>">Company Type</a>
               </li>
            </ul>
         </div>
      </div>
         ...
         <div class="col-md-12">
            <section class="tile">
               ...
               <div class="tile-header dvd dvd-btm">
                  <h1 class="custom-font">Add Company Type</h1>
                     <div class="comapny_size_add">
                  <form id="size_com" method="POST" action="<?php echo site_url('admin/master_company_type'); ?>">
                  <div class="row"> 
                  <div class="col-lg-6">            
                  <input type="text" id="myInput" class="form-control" name="name" placeholder="Enter Company Type"></div>
                   <div class="col-lg-6">     
                        ...
                        <div class="row">
                           <div class="col-sm-12">
                              <table class="table table-custom dataTable no-footer" id="editable-usage" role="grid" aria-describedby="editable-usage_info">
                                 ...
                                 <tbody>
                                   <?php 
                                   if($master_company_type!="" && count($master_company_type) >0){
                                   foreach ($master_company_type as $key => $value): ?>
  ...
  $('.edit_form_oc').submit(function(){
    ...
    if($.trim(misc) != '')
    {
      $.ajax({
        ...
        data : {table : 'master_company_type', col : 'name',misc : misc,check_col_name : 'id', check_col_data : id  }
      }).then(function(res){
        ...
        swal("ERROR", "Please enter Company Type", 'error');
    }
    return false;
  });
    ...
    $('.user_status').on('change', function(){
        ...
        $.ajax({
            method : "POST",
            url : "<?php echo site_url('admin/master_company_type_status'); ?>",
