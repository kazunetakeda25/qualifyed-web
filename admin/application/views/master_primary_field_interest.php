      ...
      <div class="pageheader">
         ...
         <div class="page-bar">
            <ul class="page-breadcrumb">
               ...
               <li>
                  <a href="<?php echo site_url('master_primary_field_interest'); ?>">User's Field Interest</a>
               </li>
            </ul>
         </div>
      </div>
         ...
         <div class="col-md-12">
            <section class="tile">
               ...
               <div class="tile-header dvd dvd-btm">
                  <h1 class="custom-font">Add User's Field Interest</h1>
                     <div class="comapny_size_add">
                  <form id="size_com" method="POST" action="<?php echo site_url('admin/master_primary_field_interest'); ?>">
                  <div class="row"> 
                  <div class="col-lg-6">            
                  <input type="text" id="myInput" class="form-control" name="name1" placeholder="Enter Company Type"></div>
                   <div class="col-lg-6">     
                        ...
                        <div class="row">
                           <div class="col-sm-12">
                              <table class="table table-custom dataTable no-footer" id="editable-usage" role="grid" aria-describedby="editable-usage_info">
                                 ...
                                 <tbody>
                                   <?php 
                                   if($master_primary_field_interest!="" && count($master_primary_field_interest) >0)
                                    {
                                   foreach ($master_primary_field_interest as $key => $value): ?>
...
<script>
    ...
    if($.trim(misc) != '')
    {
      $.ajax({
        ...
        data : {table : 'master_primary_field_interest', col : 'name',misc : misc,check_col_name : 'id', check_col_data : id  }
      }).then(function(res){
        ...
        swal("ERROR", "Please enter Primary Field Interest", 'error');
    }
    return false;
  });
    ...
    $('.user_status').on('change', function(){
        ...
        $.ajax({
            method : "POST",
            url : "<?php echo site_url('admin/master_primary_field_interest_status'); ?>",
            data : {status : status , id : $(this).data('id')}
        }).then(function(){
            swal("Update Success", "User's Field Interest update successfully",'success');
        })
    });
     $('#size_com').submit(function(e){
      if($('input[name="name1"]').val() == '')
      {
        swal("Please Enter User's Field Interest");
        return false;
      }else{
        return true;
      }
