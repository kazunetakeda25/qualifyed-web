<?php
    class Comman_model extends CI_Model {
        ...
        public function getUserReport($condition = array())
        {
            ...
            $sql = "SELECT t1.*,CONCAT(t2.fname, ' ', t2.lname) as fullname,t2.phone FROM user_login as t1 LEFT JOIN users as t2 ON t2.login_id = t1.id WHERE t1.type = 2 AND t1.status = 1 AND (date(t1.currentdate) >= '$sdate' AND date(t1.currentdate) <= '$edate') ORDER BY t1.currentdate DESC";
            $exec = $this->db->query($sql);
            $data = $exec->result_array();
            return $data;
        }
        ...
        public function getBusinessReport($condition = array())
        {
            ...
            $sql = "SELECT t1.*,t2.name as fullname,t2.phone FROM user_login as t1 LEFT JOIN bussiness_profile as t2 ON t2.loginid = t1.id WHERE t1.type = 1 AND t1.status = 1 AND (date(t1.currentdate) >= '$sdate' AND date(t1.currentdate) <= '$edate') ORDER BY t1.currentdate DESC";
            $exec = $this->db->query($sql);
            //echo $this->db->last_query();die;
            $data = $exec->result_array();
            return $data;
