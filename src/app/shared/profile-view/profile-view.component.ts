...
export class ProfileViewComponent implements OnInit {
 ...
 constructor(private http:HttpClient,private fb:FormBuilder,private rt:Router,private user:UserService) { }
  ...
  ngOnInit() {
  	...
  	this.user.post("profileviewlist",{limit:"0,5",userid:localStorage.getItem('id')}).subscribe(res=>{
  		this.resData = res;
   ...
   this.user.post("profileviewlist",{limit:((this.currentCount)+','+(5)).toString(),userid:localStorage.getItem('id')}).subscribe(res=>{
                  this.final=res
